...
es = input(f' {F}({C}1{F}) {X} 𝐒𝐞𝐚𝐬𝐨𝐧 𝐈𝐃 {F}  '+Z)
print(X+' ╸╸╸╸╸╸╸╸╸╸╸╸╸╸  ')
token = input(f' {F}({C}2{F}) {X} 𝐄𝐧𝐭𝐞𝐫 𝐓𝐨𝐤𝐞𝐧{F}  '+Z)
print(X+' ╸╸╸╸╸╸╸╸╸╸╸╸╸╸  ')
ID = input(f' {F}({C}3{F}) {X} 𝐄𝐧𝐭𝐞𝐫 𝐈𝐃{F}  '+Z)
os.system('clear')
head = {
'accept': '*/*',
...
'user-agent': 'Mozilla/5.0 (Windows NT 6.2; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/85.0.4183.83 Safari/537.36',
'x-asbd-id': '198387',
...
"username":email,"password":"paswgwgshhddv","device_id":uis,
 ...
 if "The username you entered doesn't appear to belong to an account. Please check your username and try again." in req:
 	...
 	n+=1
 	print(f''' {X}—{C}{C}—{X}—{X}—{C}—{X}—{C}—{X}—{B}×{X}—{C}—{X}—{C}—{X}—{C}—{X}—{C}—{X}—
 {F}! {F}- {F}Good     » 「{v}」
 ...
 {U}! {U}- {U}Good GM  » 「{m}」
 {X}—{C}{C}—{X}—{X}—{C}—{X}—{C}—{X}—{B}×{X}—{C}—{X}—{C}—{X}—{C}—{X}—{C}—{X}—''')
...
⋘─────━𓆩NADER𓆪‏━─────⋙
 ...
 𝖕𝖗𝖑𝖛𝖆𝖙𝖊 » [{isp}]  
 𝖇𝖑𝖔 » [{bio}]   
 𝖑𝖎𝖓𝖐 » https://www.instagram.com/{user}   
⋘─────━𓆩NADER𓆪‏━─────⋙
@nader20090  ''')
